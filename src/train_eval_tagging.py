...
def evaluate(FLAGS, sess, model, data_processor, label_list=None):
    ...
    pred_loss, pred_result, gold_result, gold_mask, r_loss = model.create_model(
        input_ids,
        input_mask,
        segment_ids,
        lmask,
    ...
    for step in range(test_num // batch_size):
        inputs, loss_value, preds, golds, gmask = sess.run(
            [input_ids, pred_loss, pred_result, gold_result, gold_mask]
        )
...
def train(FLAGS):
    ...
    test_data_processor = DataProcessor(
        test_file, max_sen_len, vocab_file, out_dir, label_list=None, is_training=False
    )
    print("test_file_merr=", test_file_merr)
    test_data_processor_merr = DataProcessor(
        test_file_merr,
        max_sen_len,
        vocab_file,
        out_dir,
    ...
    data_processor = DataProcessor(
        train_path, max_sen_len, vocab_file, out_dir, label_list=None, is_training=True
    )
    ...
    model = BertTagging(
        bert_config_path,
        num_class=len(data_processor.get_label_list()),
        max_sen_len=max_sen_len,
        alpha=alpha,
        keep_prob=keep_prob,
    )
    (loss, probs, golds, mask, r_loss) = model.create_model(
        input_ids,
        input_mask,
        segment_ids,
        lmask,
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        train_op = optimization.create_optimizer(
            loss, learning_rate, num_steps, num_warmup_steps, use_tpu=False
        )
        ...
        for epoch in range(EPOCH):
            for step in range(int(train_num / batch_size)):
                ...
                if step % 500 == 0:
                    ...
                    format_str = (
                        "Epoch {} step {},  train loss = {:.4f},{:.4f},{:.4f} ( {:.4f} examples/sec; {:.4f} "
                        "sec/batch)"
                    )
                    print(
                        format_str.format(
                            ...
                            step,
                            np.mean(loss_values),
                            np.mean(loss_values[-500:]),
                            np.mean(r_loss_values[-500:]),
                            examples_per_sec,
                            duration,
                        )
                    )
