...
def score_f(ans, print_flg=False, only_check=False, out_dir=""):
    ...
    # check p, r, f
    p = 1.0 * check_right_pred_err / (total_pred_err + 0.001)
    r = 1.0 * check_right_pred_err / (total_gold_err + 0.001)
    f = 2 * p * r / (p + r + 1e-13)
    print(
        "token num: gold_n:%d, pred_n:%d, right_n:%d"
        % (total_gold_err, total_pred_err, check_right_pred_err)
    )
    ...
    # correction p, r, f
    pc1 = 1.0 * right_pred_err / (total_pred_err + 0.001)
    pc2 = 1.0 * right_pred_err / (check_right_pred_err + 0.001)
    rc = 1.0 * right_pred_err / (total_gold_err + 0.001)
