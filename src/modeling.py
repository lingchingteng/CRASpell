    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            ...
            if position_ids is None:
                ...
                if position_ids.shape.ndims == 2:
                    position_ids = tf.expand_dims(position_ids, axis=[-1])
                position_embeddings = tf.nn.embedding_lookup(
                    full_position_embeddings, position_ids
                )
                input_shape = get_shape_list(position_ids)
                position_embeddings = tf.reshape(
                    position_embeddings, input_shape[0:-1] + [input_shape[-1] * width]
                )
