...
class Mask(object):
    def __init__(
        ...
        self.stroke_thr = (
            self.config["same_py"] + self.config["simi_py"] + self.config["stroke"]
        )
        self.random_thr = (
            self.config["same_py"]
            + self.config["simi_py"]
        ...
        self.all_token_ids = [
            int(x) for x in self.same_py_confusion.tokenizer.vocab.values()
        ]
    ...
    def do_mask(self, input_sample, target_indice):
        ...
        for pos in target_indice:
            if method == "pinyin":
                new_c = self.same_py_confusion.get_confusion_item_by_ids(
                    input_sample[pos]
                )
                if new_c is not None:
                    masked_sample[pos] = new_c
            elif method == "jinyin":
                new_c = self.simi_py_confusion.get_confusion_item_by_ids(
                    input_sample[pos]
                )
                if new_c is not None:
                    masked_sample[pos] = new_c
    ...
    def mask_process_rand(self, input_sample, label_ids):
        ...
        neg_ratio = 0.3
        valid_ids = set(
            [idx for (idx, v) in enumerate(input_sample) if v not in self.invalid_ids]
        )
    ...
    def mask_process_pos(self, input_sample, label_ids):
        ...
        for current_idx in current_indice:
            mask_positions += self.get_mask_position(
                valid_ids, check_vector, current_idx
            )
        do_mask_flg = False
